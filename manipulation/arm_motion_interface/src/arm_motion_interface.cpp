...
{ // constructor
    ...
    //pose to drop tote off on table
    q_stow_pose9_<<0.0, -1.22, 0, 1.516, 0, 1.27, 1.57;
...
//should invoke planning for joint-space trajectory to drop off gearboxes safely
...
//should invoke planning for joint-space trajectory to drop off bolts safely
...
//should invoke planning for joint-space trajectory to drop off gears safely
...
//should invoke planning for joint-space trajectory to extract arm safely from near pedestal
bool ArmMotionInterface::plan_jspace_traj_recover_from_dropoff() {
    int nsteps = cart_goal_.nsteps;
    double arrival_time = cart_goal_.arrival_time;
    //invoke general joint-space planner fnc; specify q_start = q_current and q_goal in home pose;
    //set trajectory arg to member var des_trajectory_
...
//should invoke planning for joint-space trajectory to pick up the tote from the base safely
bool ArmMotionInterface::plan_jspace_traj_current_to_tote_pickup() {
    ...
    traj_is_valid_ = pCartTrajPlanner_->plan_jspace_traj_qstart_to_qend(q_waiting_pose_, q_stow_pose9_, nsteps, arrival_time, des_trajectory_);
    if (traj_is_valid_) {
        multi_traj_vec_.push_back(des_trajectory_);
    }
