...
int main(int argc, char** argv) {
    ...
    */
    ...
    cout<<"Enter 1 to attempt grasp and stow part, 2 to place filled tote on table, 0 to quit: ";
    cin>>ans;
    ...
    if(ans == 0){
     ...
     if (ans==2) {
      ROS_INFO("Attempting to deliver tote from pedestal to table");
      success = movePart.grab_tote_from_pedestal_and_place_on_table();

  
    if(!success){
        ROS_ERROR("dropoff tote failed");
    ...
    if (ans==1) {
      ROS_INFO("Attempting to grasp and stow chosen part");
